...
tar_source()
tar_option_set(
  packages = c("sf", "terra", "dplyr", "here", "magrittr", "stringr"),
  format = "qs"
)
...
list(
  ...
  # ── Phase 1: Setup ────────────────────────────────────────────────────────
  # bowen_zoning is defined here (not in Phase 2) because bowen_mask depends
  # on it. targets resolves ordering automatically.
  ...
  # ── Phase 2: Foundation ───────────────────────────────────────────────────
  tar_target(bowen_protectedareas,     load_protected_areas(project_crs)),
  tar_target(dissolved_protectedareas, dissolve_protected_areas(bowen_protectedareas)),
  tar_target(ogma,                     load_ogma(project_crs)),
  tar_target(crown,                    load_crown(project_crs)),
  tar_target(unprotected_crown,        create_unprotected_crown(crown, dissolved_protectedareas)),
  tar_target(parcelmap_bowen,          load_parcelmap(project_crs)),
  tar_target(privateland,              create_privateland(parcelmap_bowen, dissolved_protectedareas)),
