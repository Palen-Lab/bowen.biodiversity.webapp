...
# Save plot
output_path <- here::here("vignettes/figures/feb1/01_bowen_map")
dir.create(output_path)
ggplot2::ggsave(
  paste0(output_path, "/", Sys.Date(), "_01_bowen_map.png"),
  bowen_plot,
  device = ragg::agg_png,
...
bowen_plot <- bowen_map(raster_layer = SDM_sum,
                        title = "Summed Species Distribution Models on Bowen Island",
...
# Save plot
output_path <- here::here("vignettes/figures/feb1/02_bowen_summed_sdm_map")
dir.create(output_path)
ggplot2::ggsave(
  paste0(output_path, "/", Sys.Date(), "_02_bowen_summed_sdm_map.png"),
  bowen_plot,
  device = ragg::agg_png,
  width = 9, height = 12, units = "in", res = 300
)
```
# 03 Species Richness (All)
```{r species_richness}
...
output_path <- here::here("vignettes/figures/feb1/03_bowen_species_richness_map")
dir.create(output_path)
ggplot2::ggsave(
  here(output_path, "/", paste0(Sys.Date(), "_03_bowen_species_richness_map.png")),
  total_richness_0.7_plot,
  device = ragg::agg_png,
  width = 9, height = 12, units = "in", res = 300
)
```
# 04 Habitat Layers (MVSEI)
```{r water}
...
bowen_plot <- bowen_map(raster_layer = total_richness_0.7,
                        title = title,
                        subtitle = subtitle,
                        caption = caption,
                        legend_label = "Water") 
# Save plots
output_path <- here::here("vignettes/figures/feb1/04_bowen_water_map")
dir.create(output_path)
ggplot2::ggsave(
  here(output_path, "/", paste0(Sys.Date(), "_","04_bowen_water_map", ".png")),
  bowen_plot,
  device = ragg::agg_png,
  width = 9, height = 12, units = "in", res = 300
)
...
zonation_path <- here::here("vignettes/figures/equal_weight_zonation/rankmap.tif")
zonation_rast <- terra::rast(zonation_path)
output_path <- here::here("vignettes/figures/private_land")
dir.create(output_path)

private_land <- sf::st_read(here::here("inst/app/data/bowen_island_properties"))

private_land_colour <- "#EE964B"

zonation_plot <- bowen_map(zonation_rast,
                                    title = "Biodiversity Hotspots and Protected Areas",
                                    subtitle = "Biodiversity hotspots, with gradient scale corresponding to relative priority for conservation. Higher values, towards 1.0, indicate higher priority. Lower values, towards 0.0, show lower priority. Private lands are shown overlaid ontop of the priority gradient.",
                                    caption = "Jay Matsushiba January 27 2025",
                                    legend_label = "Priority") +
  geom_sf(data = private_land, 
          aes(fill = private_land_colour, 
              color = private_land_colour), 
          alpha = 0.3,
          linewidth = 0.2) +
  scale_fill_identity(name = "",
                      breaks = c(private_land_colour),
                      labels = c("Private Lands"),
                      guide = "legend") +
  scale_colour_identity(name = "",
                        breaks = c(private_land_colour),
                        labels = c("Private Lands"),
...
ggplot2::ggsave(
  paste0("figures/private_land/", Sys.Date(), "_private_land_overlap.png"),
  zonation_plot,
  device = ragg::agg_png,
  width = 9, height = 12, units = "in", res = 300
)
