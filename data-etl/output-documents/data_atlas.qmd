...
knitr::opts_chunk$set(
  ...
  out.width = "100%" # Scale to full page width (works for both PDF and Word)
)
```
...
## Total Habitat Diversity: Number of Habitat Types by Location
...
# Protected Areas
## Biodiversity Value with Protected Areas
...
# Overlays
## Top 30% Biodiversity Value with Human Disturbance: Continuous
## Top 30% Biodiversity Value with Wildfire Vulnerability Index
## Top 30% Biodiversity Value with Protected Areas 
## Top 30% Biodiversity Value with Private Land
...
caption <- paste0(
  "Map created: ",
  date(),
  ". Palen Lab. Created by Jay Matsushiba (jmatsush@sfu.ca)"
)
...
writeRaster(
  total_richness,
  here("inst/extdata/2_species/total_richness.tif"),
  overwrite = T
)
...
total_richness_plot <- bowen_map(
  raster_layer = total_richness,
  title = title,
  subtitle = subtitle,
  caption = caption,
  legend_label = "Richness"
...
ggplot2::ggsave(
  here(
    output_dir,
    "/",
    paste0(Sys.Date(), "_2_5_bowen_species_richness_map.png")
  ),
  total_richness_plot,
  device = ragg::agg_png,
  width = 9,
...
fw_plot <- bowen_map(
  ...
  tidyterra::geom_spatraster(
    data = rast(unlist(fw_df[fw_df$name == "Lakes", "path"])),
    aes(fill = layer)
  ) +
  ...
  tidyterra::geom_spatraster(
    data = rast(unlist(fw_df[fw_df$name == "Ponds", "path"])),
    aes(fill = layer)
  ) +
  scale_fill_gradientn(
    name = "",
  ...
  ggnewscale::new_scale_fill() +
  tidyterra::geom_spatraster(
    data = rast(unlist(fw_df[fw_df$name == "Riparian", "path"])),
    aes(fill = layer)
  ) +
  scale_fill_gradientn(
    name = "",
  ...
  ggnewscale::new_scale_fill() +
  tidyterra::geom_spatraster(
    data = rast(unlist(fw_df[fw_df$name == "Wetlands", "path"])),
    aes(fill = layer)
  ) +
  scale_fill_gradientn(
    name = "",
...
writeRaster(
  fw_richness,
  here("inst/extdata/3_habitats/fw_richness.tif"),
  overwrite = T
)

fw_richness_plot <- bowen_map(
  raster_layer = fw_richness,
  title = title,
  subtitle = subtitle,
  caption = caption,
  pal = "Mako",
...
ggplot2::ggsave(
  here(
    output_dir,
    "/",
    paste0(Sys.Date(), "_", "3_1_bowen_water_richness_map", ".png")
  ),
  fw_richness_plot,
  device = ragg::agg_png,
  width = 9,
...
writeRaster(
  habitat_richness,
  here("inst/extdata/3_habitats/total_habitat_richness.tif"),
  overwrite = T
)
# Create species richness plots:
## Each species is marked present in a cell, if they are above a certain
...
habitats_plot <- bowen_map(
  raster_layer = habitat_richness,
  title = title,
  subtitle = subtitle,
  caption = caption,
  legend_label = "Richness"
...
ggplot2::ggsave(
  here(
    output_dir,
    "/",
    paste0(Sys.Date(), "_", "3_5_bowen_habitat_richness_map", ".png")
  ),
  habitats_plot,
  device = ragg::agg_png,
  width = 9,
...
# Plot
combined_biodiversity_plot <- bowen_map(
  raster_layer = rankmap,
...
ggplot2::ggsave(
  paste0(
    ...
    Sys.Date(),
    "_",
    "4_1_combined_biodiversity_values.png"
  ),
...
disturbed <- here::here(
  ...
  "extdata",
  "habitats",
  "bowen_human_footprint.tif"
) %>%
  terra::rast() %>%
  project(project_crs)

zonation_plot <- bowen_map(
  disturbed,
...
ggplot2::ggsave(
  paste0(
    ...
    Sys.Date(),
    "_",
    "4_2_humandisturbance_continuous",
    ".png"
  ),
  zonation_plot,
  device = ragg::agg_png,
...
bowen_rasterize_cover <- function(sf) {
  ...
  output <- sf %>%
    ...
    terra::rasterize(
      ...
      background = NA
    ) # Rasterize to match mask, set background values to NA
}
...
bowen_protectedareas_2 <- st_read(here(
  "data-raw/archive/bowen_island_protectedareas/BM_PROTECTED_AREAS.shp"
)) %>%
...
foreach(quant = quants) %do%
  {
    ...
    protectedareas_plot <- bowen_map_toppct(
      ...
      subtitle = paste0(
        "Top ",
        top_pct,
        "% of biodiversity locations on Bowen Island, with gradient scale corresponding to relative priority for conservation. Higher values, towards 1.0, indicate higher priority. Lower values, towards 0.0, show lower priority. Protected Areas are visualized as an overlay on top of the Zonation priotization output. The percentage of these biodiversity locations covered by Protected Areas is ",
    ...
    privateland_plot <- bowen_map_toppct(
      ...
      subtitle = paste0(
        "Top ",
        top_pct,
        "% of biodiversity locations on Bowen Island, with gradient scale corresponding to relative priority for conservation. Higher values, towards 1.0, indicate higher priority. Lower values, towards 0.0, show lower priority. Private Lands are visualized as an overlay on top of the Zonation priotization output. The percentage of these biodiversity locations covered by Private Lands is ",
    ...
    bowen_hfp_plot <- bowen_map_2(
      ...
      subtitle = paste0(
        "Top ",
        top_pct,
        "% of biodiversity locations on Bowen Island, with scale corresponding to relative human disturbance. 0% of top biodiversity locations are considered intact. ",
        hfp_low_overlap * 100,
        "% of top biodiversity locations considered to be less disturbed. ",
        hfp_med_overlap * 100,
        "% of top biodiversity cells are considered moderately disturbed. ",
        hfp_high_overlap * 100,
        "% of top biodiversity locations are highly disturbed. Citation: Kristen Hirsh-Pearson, Chris J. Johnson, Richard Schuster, Roger D. Wheate, and Oscar Venter. 2022. Canada’s human footprint reveals large intact areas juxtaposed against areas under immense anthropogenic pressure. FACETS. 7: 398-419."
      ),
      caption = caption,
      legend_label = "Disturbance"
    ) +
...
foreach(quant = quants) %do%
  {
    ...
    bowen_hfp_plot <- bowen_map_2(
      ...
      subtitle = paste0(
        ...
        hfp_high_overlap * 100,
        "% of bottom biodiversity locations are highly disturbed. Citation: Kristen Hirsh-Pearson, Chris J. Johnson, Richard Schuster, Roger D. Wheate, and Oscar Venter. 2022. Canada’s human footprint reveals large intact areas juxtaposed against areas under immense anthropogenic pressure. FACETS. 7: 398-419."
      ),
      caption = caption,
      legend_label = "Disturbance"
    ) +
