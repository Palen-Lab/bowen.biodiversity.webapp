...
This document contains the code for generating the figures contained in the Data Atlas, using the May 2nd 2025 version of inputs and outputs. 
...
4.  Derived Data Products
    ...
    9.  Top 30% overlap with Human disturbance (% in each of 3 categories)
...
```{r}
rankmap <- here("inst/extdata/output_zonation/2025-05-02/rankmap.tif") %>% 
  rast() %>% 
  project(project_crs)
...
disturbed <- here::here("inst", "extdata", "bowen_human_footprint.tif") %>%
  terra::rast()
...
```{r}
# Human Footprint
disturbed_classified <- here::here("inst/extdata/bowen_human_footprint_recl.tif") %>%
  terra::rast() %>%
  as.factor()
...
# TODO: go back to polygons for input polygons (protected areas, private lands)
# TODO: or go back to raster generation 
...
# Human Footprint (continuous)
bowen_hfp_continuous <- here("inst/extdata/habitats/bowen_human_footprint.tif") %>%
  rast()
# Human Footprint (categorical)
...
foreach(quant = quants[1]) %do% {
  ...
  top_plot <- bowen_map(rankmap_bt_rm,
  ...
  protectedareas_overlap <- raster_overlap_ratio(rankmap_bt_rm, bowen_protectedareas_rast) %>%
    round(2)
  protectedareas_plot <- bowen_map(rankmap_bt_rm,
  ...
  privatelands_overlap <- raster_overlap_ratio(rankmap_bt_rm, bowen_parcelmap_rast) %>%
    round(2)
  privateland_plot <- bowen_map(rankmap_bt_rm,
  ...
  bowen_hfp_top <- rankmap_mask * bowen_hfp
  ...
  bowen_hfp_continuous_top <- rankmap_mask * bowen_hfp
  ...
  fw_plot <- bowen_map(empty_rast,
    ...
    scale_fill_gradientn(name = "",
                         ...
                         na.value = "transparent") +
    ggnewscale::new_scale_fill() +
    tidyterra::geom_spatraster(data = fw_mask,
                               aes(fill = sum)) +
    scale_fill_gradientn(name = "",
                         labels = "Freshwater Only",
                         colours = "aquamarine",
                         guide = "legend",
                         na.value = "transparent") +
    ggnewscale::new_scale_fill() +
    tidyterra::geom_spatraster(data = fw_top,
                               aes(fill = rankmap)) +
    scale_fill_gradientn(name = "",
                         labels = paste0("Freshwater / Top ", top_pct, "% Overlap"),
                         colours = "blue",
                         guide = "legend",
                         na.value = "transparent"
    ) + 
