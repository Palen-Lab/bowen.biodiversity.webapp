...
config <- list(
  ...
  run_foundation = FALSE, # Roads, trails, parcels, protected areas, etc.
  ...
  run_habitats = FALSE, # TEM, SEI, consultant data → ~50 layers

  # Phase 5: Zonation (20-40 minutes)
  run_zonation = FALSE, # Conservation prioritization analysis

  # Phase 6: Threats (10-15 minutes)
  run_threats = FALSE, # Development, fire, climate overlays
...
if (config$run_setup) {
  message("\n=== PHASE 1: Setup ===")
  message(
    "Creating base layers: CRS, boundary, mask, roads, trails, shoreline..."
  )

  tryCatch(
    {
      scripts <- list.files(
        here("data-raw/scripts/01_setup"),
        pattern = "\\.R$",
        full.names = TRUE
      )
...
if (config$run_foundation) {
  message("\n=== PHASE 2: Foundation Data ===")
  message(
    "Processing context layers: parcels, protected areas, zoning, wetlands, fish streams..."
  )

  tryCatch(
    {
      scripts <- list.files(
        here("data-raw/scripts/02_foundation"),
        pattern = "\\.R$",
        full.names = TRUE
      )
      scripts <- sort(scripts) # Ensure numbered order
...
if (config$run_species) {
  ...
  tryCatch(
    {
      scripts <- list.files(
        here("data-raw/scripts/03_species"),
        pattern = "\\.R$",
        full.names = TRUE
      )
...
if (config$run_habitats) {
  ...
  tryCatch(
    {
      scripts <- list.files(
        here("data-raw/scripts/04_habitats"),
        pattern = "\\.R$",
        full.names = TRUE
      )
      scripts <- sort(scripts)
...
if (config$run_threats) {
  ...
  tryCatch(
    {
      scripts <- list.files(
        here("data-raw/scripts/06_threats"),
        pattern = "\\.R$",
        full.names = TRUE
      )
      scripts <- sort(scripts)
