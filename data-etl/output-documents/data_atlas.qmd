...
# Protected Areas (to subtract from Crown Land)
...
# Remove polygons that should not be included
...
# Create binary masks for each ownership category
private_mask <- comb_rast %>%
  subst(2, NA) %>%
  subst(3, NA) %>%
  subst(4, NA)
protected_mask <- comb_rast %>%
  subst(1, NA) %>%
  subst(3, NA) %>%
  subst(4, NA)
crown_mask <- comb_rast %>%
  subst(1, NA) %>%
  subst(2, NA) %>%
  subst(4, NA)
mixed_mask <- comb_rast %>%
  subst(1, NA) %>%
  subst(2, NA) %>%
  subst(3, NA)

# Overlap of each category with top 30% biodiversity
private_overlap <- raster_overlap_ratio(private_mask, rankmap_mask) %>% round(3)
protected_overlap <- raster_overlap_ratio(protected_mask, rankmap_mask) %>%
  round(3)
crown_overlap <- raster_overlap_ratio(crown_mask, rankmap_mask) %>% round(3)
mixed_overlap <- raster_overlap_ratio(mixed_mask, rankmap_mask) %>% round(3)
