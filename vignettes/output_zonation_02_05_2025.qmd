...
foreach(i=1:nrow(layers_df[1:10,])) %do% {
...
```{r}
# Sanity check on the overlap calculations 
# top <- rankmap %>%
#   remove_by_quantile(0.9)
# ggplot() +
#   geom_spatraster(data = bowen_protectedareas_rast) +
#   scale_fill_gradient(low = "red", high = "red") +
#   new_scale_fill() +
#   geom_spatraster(data = top) +
#   scale_fill_gradientn(colors = alpha(c("white", "white"), alpha = .5), na.value = NA) 
```
