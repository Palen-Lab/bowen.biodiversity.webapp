...
# Output Directory
output_dir <- here("output-figures/threats_development")
...
# Template raster
bowen_mask <- terra::rast(here("data/bowen_mask.tif"))
...
```{r}
parcelmap <- st_read(here("data-raw/datasets/parcelmap/bowen_parcelmap.gpkg")) %>%
...
parcelmap_extract <- terra::extract(biod_vals, parcelmap_vect, sum, bind = T)

parcelmap_sf <- st_as_sf(parcelmap_extract)

ggplot(parcelmap_sf) + geom_sf(aes(fill = rankmap))
