...
if (!dir.exists(output_dir_annotated)) {
  dir.create(output_dir_annotated)
}
if (!dir.exists(output_dir_unannotated)) {
  dir.create(output_dir_unannotated)
}
...
```{r data_imports}
#### Raster Data ####
# Biodiversity values (Zonation prioritization output)
# Used in: fw_habitats_top30, biodiversity_values, biodiversity_values_top30,
#          wildfire_vulnerability_top30, protected_areas_top30,
#          biod_vals_private_land, biod_vals_public_land,
#          land_ownership, land_ownership_top30
...
# Used in: wildfire_vulnerability, wildfire_vulnerability_top30
fire_index <- rast(here("output-data/6_threats/fire_index_40m.tif"))

#### Vector Data ####
# Bowen Island shoreline
# Used in: protected_areas_top30, biod_vals_private_land, biod_vals_public_land,
#          land_ownership, land_ownership_top30
...
# Wildland Urban Interface classification (used in: wildland_urban_interface)
bowen_wui_raw <- here("data-raw/datasets/bc_wui/wui.gpkg") %>%
  st_read(quiet = T)

#### Package Datasets (loaded via devtools::load_all()) ####
# Available: crown, parcelmap_bowen, bowen_protectedareas,
#            bowen_ocean_sf, bowen_roads, bowen_trails, ogma

#### Common Derived Objects ####
# Protected areas
# Used in: protected_areas_top30, biod_vals_public_land,
#          land_ownership, land_ownership_top30
protectedareas <- bowen.biodiversity.webapp::bowen_protectedareas %>%
  filter(type != "Agricultural Land Reserve")
...
# Crown land excluding protected areas (Other Public Lands)
# Used in: biod_vals_public_land, land_ownership, land_ownership_top30
crown_valid <- crown %>%
...
# Exclude: rowname 1 (Fairy Fen Natural Reserve, sliver),
#          rowname 74 (Seymour Bay Park, invalid polygon)
notprotected_crown <- notprotected_crown[
  !rownames(notprotected_crown) %in% c("1", "74"),
]

parcelmap_private <- parcelmap_bowen %>%
  filter(OWNER_TYPE %in% c("Private", "Unclassified"))
# Private land raster
# Used in: biod_vals_private_land, land_ownership, land_ownership_top30
private_raster <- parcelmap_private %>%
  bowen_rasterize() %>%
  project(rankmap_bt_rm)

# Freshwater layer metadata
# Used in: fw_habitats (section 4.1), fw_habitats_top30 (section 4.4)
...
ggsave_drive(
  here(output_dir_annotated, "2_1_transportation_network.png"),
  basic_plot,
...
ggsave_drive(
  here(output_dir_unannotated, "2_1_transportation_network_no_annotation.png"),
...
Based on >`{r} presence_threshold` probability of occurrence in Species Distribution Models.
```{r species_richness}
#### Preparing Raster ####
# List SDMs filepaths
...
# Save plot
ggsave_drive(
  here(output_dir_annotated, "3_1_species_richness.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "3_1_species_richness_no_annotation.png"),
...
```{r bird_species_richness, eval=F}
...
ggsave_drive(
  here(output_dir_annotated, "3_2_bird_species_richness.png"),
  bird_richness_plot,
  drive_folder_id_annotated,
  device = ragg::agg_png,
  width = 9,
...
ggsave_drive(
  here(output_dir_unannotated, "3_2_bird_species_richness_no_annotation.png"),
  bird_richness_plot + template_plot_remove(),
  drive_folder_id_unannotated,
  device = ragg::agg_png,
  width = 9,
  height = 12,
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "4_1_freshwater_habitats.png"),
  fw_plot_annotation,
  drive_folder_id_annotated,
  device = ragg::agg_png,
  width = 9,
...
ggsave_drive(
  here(output_dir_unannotated, "4_1_freshwater_habitats_no_annotation.png"),
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "4_2_freshwater_richness.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "4_2_freshwater_richness_no_annotation.png"),
...
# Saving plot
ggsave_drive(
  here(output_dir_annotated, "4_3_habitat_richness.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "4_3_habitat_richness_no_annotation.png"),
...
## Freshwater Habitats: Top 30% Biodiversity Values
Top 30% of biodiversity locations on Bowen Island, showing freshwater habitat features within the top 30% biodiversity cells. Cells in the top 30% without freshwater features are shown in grey. 46.7% of all cells contain freshwater habitats. The percentage of top 30% biodiversity locations covered by freshwater habitats is 56.6%.
```{r fw_habitats_top30}
#### Preparing layers for plotting ####
# Grey raster for top 30% background
grey_rast <- rankmap_mask %>%
  project("EPSG: 3857") %>%
  higher_res_edges() %>%
  as.factor()

# Helper: load raster, mask to top 30%, and prep for map
prep_rast_top30 <- function(path) {
  path %>%
    unlist() %>%
    rast() %>%
    project(rankmap_bt_rm) %>%
...
# Streams need special handling (binary presence)
streams_rast_top30 <- fw_df[fw_df$name == "Streams", "path"] %>%
  unlist() %>%
  rast() %>%
  not.na() %>%
  classify(cbind(FALSE, NA)) %>%
...
# Combined freshwater presence mask (for statistics)
fw_paths <- fw_df$path %>% unlist()
fw_presence_list <- lapply(fw_paths, function(p) {
  rast(p) %>%
    project(rankmap_bt_rm) %>%
    not.na() %>%
    classify(cbind(FALSE, NA))
})
fw_presence_mask <- rast(fw_presence_list) %>%
  sum(na.rm = TRUE) %>%
  classify(cbind(0, NA)) %>%
  not.na() %>%
  classify(cbind(FALSE, NA))
...
#### Statistics ####
# Proportion of all cells that are freshwater habitat
n_cells_fw <- fw_presence_mask %>%
  global("sum", na.rm = TRUE)
n_cells_total <- rankmap %>%
  not.na() %>%
  global("sum", na.rm = TRUE)
n_cells_fw / n_cells_total

# Proportion of top 30% biodiversity cells overlapping with freshwater
n_cells_top30bv_fw <- (fw_presence_mask * rankmap_mask) %>%
  global("sum", na.rm = TRUE)
n_cells_top30bv <- rankmap_mask %>%
  global("sum", na.rm = TRUE)
n_cells_top30bv_fw / n_cells_top30bv

#### Plotting ####
fw_top30_plot_base <- template_plot +
  tidyterra::geom_spatraster(
    data = grey_rast
  ) +
  scale_fill_manual(
  ...
  ggnewscale::new_scale_fill() +
  tidyterra::geom_spatraster(
    data = prep_rast_top30(fw_df[fw_df$name == "Lakes", "path"]),
    aes(fill = layer)
  ) +
  scale_fill_gradientn(
    name = "",
  ...
  ggnewscale::new_scale_fill() +
  tidyterra::geom_spatraster(
    data = prep_rast_top30(fw_df[fw_df$name == "Ponds", "path"]),
    aes(fill = layer)
  ) +
  scale_fill_gradientn(
    name = "",
  ...
  ggnewscale::new_scale_fill() +
  tidyterra::geom_spatraster(
    data = prep_rast_top30(fw_df[fw_df$name == "Riparian", "path"]),
    aes(fill = layer)
  ) +
  scale_fill_gradientn(
    name = "",
  ...
  ggnewscale::new_scale_fill() +
  tidyterra::geom_spatraster(
    data = prep_rast_top30(fw_df[fw_df$name == "Wetlands", "path"]),
    aes(fill = layer)
  ) +
  scale_fill_gradientn(
    name = "",
  ...
  ggnewscale::new_scale_fill() +
  tidyterra::geom_spatraster(data = streams_rast_top30, aes(fill = layer)) +
  scale_fill_gradientn(
    name = "",
    labels = "Streams",
    colours = unlist(fw_df[fw_df$name == "Streams", "colour"]),
...
fw_top30_plot_annotation <- fw_top30_plot_base +
  template_plot_annotations()
fw_top30_plot_no_annotation <- fw_top30_plot_base +
  template_plot_remove()
ggsave_drive(
  here(output_dir_annotated, "4_4_freshwater_top30.png"),
  fw_top30_plot_annotation,
  drive_folder_id_annotated,
  device = ragg::agg_png,
  width = 9,
  height = 12,
...
ggsave_drive(
  here(output_dir_unannotated, "4_4_freshwater_top30_no_annotation.png"),
  fw_top30_plot_no_annotation,
  drive_folder_id_unannotated,
  device = ragg::agg_png,
  width = 9,
  height = 12,
...
# Save plot
ggsave_drive(
  here(output_dir_annotated, "5_1_biodiversity_values.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "5_1_biodiversity_values_no_annotation.png"),
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "5_2_biodiversity_values_top30.png"),
  top_plot_annotation,
  drive_folder_id_annotated,
  device = ragg::agg_png,
  width = 9,
...
ggsave_drive(
  here(
    ...
    "5_2_biodiversity_values_top30_no_annotation.png"
  ),
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "6_1_human_disturbance.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "6_1_human_disturbance_no_annotation.png"),
...
bowen_hfp_class <- here(
  "output-data/habitats/bowen_human_footprint_recl.tif"
) %>%
  rast() %>%
  project(rankmap) %>%
  as.factor()
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "6_2_human_disturbance_top30.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "6_2_human_disturbance_top30_no_annotation.png"),
...
```{r wildfire_vulnerability}
#### Preparing raster for plotting ####
fire_index_mp <- fire_index %>%
  project("EPSG: 3857") %>%
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "6_3_wildfire_vulnerability.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "6_3_wildfire_vulnerability_no_annotation.png"),
...
```{r wildfire_vulnerability_top30}
#### Preparing raster for plotting ####
top30_wvi <- fire_index %>%
  project("EPSG: 3857") %>%
  terra::subst(4294967296, NA) %>% # Max value should be NA
  terra::scale_linear() %>%
  remove_by_quantile(0.7)
...
top30_wvi_resamp <- top30_wvi %>%
  resample(rankmap_top30_mask, method = "average") %>%
  mask(rankmap_top30_mask)
top30_wvi_resamp %>%
  terra::not.na() %>%
  values() %>%
  sum()
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "6_4_wildfire_vulnerability_top30.png"),
...
ggsave_drive(
  here(
    output_dir_unannotated,
    "6_4_wildfire_vulnerability_top30_no_annotation.png"
  ),
...
```{r wildland_urban_interface}
#### Prepare layers for plotting ####
bowen_wui <- bowen_wui_raw %>%
  select(Name, PSTA) %>%
  rename(WUI = Name) %>%
...
wui_factor_cols <- c("#fff9cfff", "lightblue", "#fdef86ff", "#fcbb3aff", "#d95f0e")
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "6_5_wildland_urban_interface.png"),
  wui_plot_annotation,
  drive_folder_id_annotated,
  device = ragg::agg_png,
  width = 9,
...
ggsave_drive(
  here(
    output_dir_unannotated,
    "6_5_wildland_urban_interface_no_annotation.png"
  ),
...
parcelmap_biod_vals_plot_base <- template_plot +
  ...
  scale_fill_continuous(
    ...
    breaks = c(
      min(parcelmap_extract_sf$rankmap, na.rm = T),
      max(parcelmap_extract_sf$rankmap, na.rm = T)
    ),
    labels = c("Lower", "Higher"),
    guide = ggplot2::guide_colourbar(
      order = 1,
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "7_1_parcel_biodiversity_values.png"),
...
ggsave_drive(
  here(
    output_dir_unannotated,
    "7_1_parcel_biodiversity_values_no_annotation.png"
  ),
...
```{r subdivision_potential}
#### Preparing layers for plotting ####
# RR1 and RR2 (Rural Residential 1 and 2 Zoning)
rr1_rr2 <- here(
  "data-raw/datasets/development_potential_danielmartin/zoning subdivision potential.xlsx"
) %>%
...
write_sf(
  ...
  here(
    "data-raw/datasets/development_potential_danielmartin/ocp_subdivision_parcelmap.gpkg"
  ),
  delete_layer = TRUE
)
...
subdivide_plot_base <- template_plot +
  geom_sf(
    data = parcelmap_bowen[
      parcelmap_bowen$OWNER_TYPE %in% c("Private", "Unclassified"),
    ],
    aes(geometry = geom),
    color = NA,
    fill = "grey"
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "7_2_subdivision_potential.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "7_2_subdivision_potential_no_annotation.png"),
...
# 0.2149664
sum(st_area(union_protectedareas)) / sum(st_area(bowen_island))
# 0.2267595
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "7_3_protected_areas_top30.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "7_3_protected_areas_top30_no_annotation.png"),
...
Top 30% of biodiversity locations on Bowen Island, with gradient scale corresponding to relative priority for conservation. Private Lands are visualized as an overlay on top of the Zonation priotization output. Private lands cover 48.7% of Bowen Island. The percentage of top 30% biodiversity locations covered by Private Lands is 49.1%
...
privatelands_overlap_plot_no_annotation <- privatelands_overlap_plot_base +
  template_plot_remove()
ggsave_drive(
  here(output_dir_annotated, "7_4_private_land_top30.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "7_4_private_land_top30_no_annotation.png"),
...
## Other Public Lands: Top 30% Biodiversity Values
Top 30% of biodiversity locations on Bowen Island, with gradient scale corresponding to relative priority for conservation. Other public lands, such as crown land, are visualized as an overlay on top of the Zonation prioritization output. The total coverage of Other Public Lands on Bowen Island is 30.0%. The percentage of top 30% biodiversity locations covered by other public lands is 13.3%. 
```{r biod_vals_public_land}
#### Preparing layers for plotting ####
rankmap_for_map <- rankmap %>%
  project("EPSG: 3857") %>%
  higher_res_edges()
rankmap_bt_rm_for_map <- rankmap_bt_rm %>%
  project("EPSG: 3857") %>%
  higher_res_edges()
notprotected_crown_union <- notprotected_crown %>%
  st_make_valid() %>%
  st_union()

#### Statistics ####
notprotected_crown_rast <- notprotected_crown_union %>%
  vect() %>%
  project(rankmap_mask) %>%
  rasterize(rankmap_bt_rm, cover = TRUE)
n_cells_top30bv_crown <- (notprotected_crown_rast * rankmap_mask) %>%
  global("sum", na.rm = T)
n_cells_top30bv <- rankmap_mask %>%
  global("sum", na.rm = T)
n_cells_top30bv_crown / n_cells_top30bv

# Check with polygon / polygon area calculations for full Bowen Island
sum(st_area(notprotected_crown_union)) / sum(st_area(bowen_island))

#### Plotting ####
crown_colour <- "green"

crownland_overlap_plot_base <- template_plot +
  geom_spatraster(data = rankmap_bt_rm_for_map) +
  colorspace::scale_fill_continuous_sequential(
    na.value = NA,
    palette = "ag_GrnYl",
...
crownland_overlap_plot_annotation <- crownland_overlap_plot_base +
  template_plot_annotations()
crownland_overlap_plot_no_annotation <- crownland_overlap_plot_base +
  template_plot_remove()
ggsave_drive(
  here(output_dir_annotated, "7_5_public_land_top30.png"),
  crownland_overlap_plot_annotation,
  drive_folder_id_annotated,
  device = ragg::agg_png,
  width = 9,
  height = 12,
...
ggsave_drive(
  here(output_dir_unannotated, "7_5_public_land_top30_no_annotation.png"),
  crownland_overlap_plot_no_annotation,
  drive_folder_id_unannotated,
  device = ragg::agg_png,
  width = 9,
  height = 12,
...
# Protected Areas raster (coded as 2)
protectedareas_rast <- protectedareas %>%
  bowen_rasterize() %>%
  project(rankmap_bt_rm) %>%
  subst(1, 2) %>%
  as.factor()

# Crown Land raster (coded as 3)
...
authority_plot_no_annotations <- authority_plot_base +
  template_plot_remove()
ggsave_drive(
  here(output_dir_annotated, "7_6_land_ownership.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "7_6_land_ownership_no_annotation.png"),
...
This map shows the land ownership or authority classification for only the top 30% of biodiversity locations on Bowen Island. Of the top 30% biodiversity cells, private lands entirely cover 41.5%. Protected areas entirely cover 15.6%. Other public lands, such as crown land, entirely cover 11.5%. 31.4% of top biodiversity cells have some mix of private and public land categories.

```{r land_ownership_top30}
#### Preparing layers for plotting ####
rankmap_for_map <- rankmap %>%
  project("EPSG: 3857") %>%
  higher_res_edges()
...
# Mask to top 30% biodiversity values only
comb_rast_top30 <- mask(comb_rast, rankmap_mask) %>%
  as.factor()
comb_rast_top30_for_map <- comb_rast_top30 %>%
  project("EPSG: 3857") %>%
  higher_res_edges() %>%
  as.factor()

#### Statistics ####
sum_stats <- comb_rast_top30 %>%
  freq() %>%
  as.data.frame() %>%
  mutate(prop = count / sum(count))
top30_total <- comb_rast_top30 %>%
  not.na() %>%
  values() %>%
  sum()
...
authority_plot_no_annotations <- authority_plot_base +
  template_plot_remove()
ggsave_drive(
  here(output_dir_annotated, "7_7_land_ownership_top30.png"),
  authority_plot_annotations,
  drive_folder_id_annotated,
  device = ragg::agg_png,
  width = 9,
...
ggsave_drive(
  here(output_dir_unannotated, "7_7_land_ownership_top30_no_annotation.png"),
  authority_plot_no_annotations,
  drive_folder_id_unannotated,
  device = ragg::agg_png,
  width = 9,
...
bowen_inat_count_rast <- terra::rasterize(
  bowen_inat_vect,
  bowen_mask,
  fun = "count"
) %>%
...
# Save plots
ggsave_drive(
  here(output_dir_annotated, "8_1_inaturalist.png"),
...
ggsave_drive(
  here(output_dir_unannotated, "8_1_inaturalist_no_annotation.png"),
