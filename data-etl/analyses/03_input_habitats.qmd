...
# Create combined raster rather than separate layers, for mapping
bowen_TEM_rast <- bowen_TEM %>%
  vect() %>% 
  terra::project(project_crs) %>% # Reproject to the mask CRS
  terra::rasterize(bowen_island_mask,
                   touches = T,
                   field = "SITEMC_S1_",
                   background = NA) # Rasterize to match mask, set background values to NA
writeRaster(bowen_TEM_rast, here("inst/extdata/3_habitats/other_tem.tif"))
bowen_TEM_habitat_types <- bowen_TEM[,c("SITEMC_S1_", "SITE_S1_LA")] %>% 
  st_drop_geometry() %>% 
  unique() %>% 
  arrange(SITEMC_S1_) %>%
  distinct(SITEMC_S1_, .keep_all = T)
saveRDS(bowen_TEM_habitat_types, here("inst/extdata/3_habitats/other_tem_types.rds"))
...
# Create combined raster rather than separate layers, for mapping
bowen_sei_no_tem_rast <- bowen_sei_no_tem %>%
  vect() %>% 
  terra::project(project_crs) %>% # Reproject to the mask CRS
  terra::rasterize(bowen_island_mask,
                   touches = T,
                   field = "SEsubcl_1",
                   background = NA) # Rasterize to match mask, set background values to NA
writeRaster(bowen_sei_no_tem_rast, here("inst/extdata/3_habitats/intertidal.tif"))
```
## Bowen Heron Watch
